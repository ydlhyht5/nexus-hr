...
import { UserPlus, Calendar, Check, X, Pencil, Calculator, Save, User, KeyRound, Briefcase, DollarSign, Clock, Trash2, LockKeyhole, AlertTriangle, BarChart3, TrendingUp, Search, ChevronRight, RefreshCw } from 'lucide-react';
    ...
    <tr className={`border-b border-white/5 transition-colors group ${isNotJoined ? 'opacity-40 bg-black/20' : 'hover:bg-white/5'}`}>
            ...
            <div className="flex flex-col gap-1">
                ...
                {leaveDays > 0 && (
                    <div className="text-[10px] text-red-400">
                        请假: {leaveDays}天 (扣 ¥{Math.round(strictLeaveDeduction)})
                    </div>
                )}
            </div>
        )}
      ...
      <td className="p-4">
         <div className={`flex items-center gap-2 bg-black/20 rounded-lg p-1 border border-white/5 w-24 ${isNotJoined ? 'pointer-events-none opacity-50' : ''}`}>
             <input 
                type="number"
                className="bg-transparent border-none text-sm text-nexus-accent font-bold w-full outline-none text-center p-0 appearance-none placeholder-gray-700"
                placeholder="0"
                min="0"
                value={manualDays}
...
export const AdminDashboard: React.FC<AdminDashboardProps> = (props) => {
  ...
  const [leaveDeleteTargetId, setLeaveDeleteTargetId] = useState<string | null>(null);
  ...
  const handleDeleteLeave = (id: string) => {
    setLeaveDeleteTargetId(id);
  };

  const handleConfirmDeleteLeave = () => {
    if (leaveDeleteTargetId) {
      onDeleteLeave(leaveDeleteTargetId);
      addToast('记录已永久删除', 'success');
      setLeaveDeleteTargetId(null);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-nexus-dark text-nexus-text p-4 md:p-8">
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <header className="flex flex-col md:flex-row justify-between items-center mb-10 gap-6 border-b border-white/5 pb-6">
          ...
          <div className="flex items-center gap-4">
            <Button variant="danger" onClick={onFactoryReset} size="sm" className="text-xs px-4" icon={<RefreshCw size={12}/>}>
               重置系统
            </Button>
              ...
              <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {employees.map(emp => {
                          ...
                          return (
                              <NeonCard key={emp.id} className="cursor-default">
            ...
            {activeTab === 'leaves' && (
              <div className="max-w-4xl mx-auto space-y-6">
                 ...
                 {paginatedLeaves.length === 0 ? (
                   ...
                   <div className="space-y-4 animate-slide-up duration-500">
      ...
      <Modal isOpen={!!leaveDeleteTargetId} onClose={() => setLeaveDeleteTargetId(null)} title="删除记录确认" footer={
        <div className="flex gap-4">
            <Button variant="ghost" onClick={() => setLeaveDeleteTargetId(null)} className="flex-1">取消</Button>
            <Button variant="danger" onClick={handleConfirmDeleteLeave} className="flex-1 bg-gradient-to-r from-red-900 to-red-950 border border-red-900 hover:from-red-800 hover:to-red-900 text-red-100 shadow-lg shadow-red-950/50">确认彻底删除</Button>
        </div>
      }>
        <div className="text-center space-y-4">
            <div className="w-16 h-16 bg-gradient-to-br from-red-900/30 to-black rounded-full flex items-center justify-center mx-auto border border-red-900/50 shadow-[0_0_30px_rgba(127,29,29,0.4)] relative group">
                <div className="absolute inset-0 bg-red-500/20 blur-xl rounded-full animate-pulse-slow"></div>
                <Trash2 size={32} className="text-red-500 relative z-10" />
            </div>
            <div>
                <h4 className="text-xl font-bold text-white mb-2">删除警告</h4>
                <p className="text-nexus-muted text-sm">您确定要彻底移除这条请假记录吗？</p>
            </div>
            <div className="bg-red-950/20 border border-red-900/20 p-4 rounded-xl text-left text-sm text-red-200/60 backdrop-blur-sm">
                <div className="flex items-start gap-3">
                    <AlertTriangle size={16} className="text-red-400 mt-0.5 shrink-0" />
                    <div className="space-y-1">
                        <p>此操作将永久删除该条记录数据。</p>
                        <p>删除后无法恢复，且员工端也不再显示。</p>
                    </div>
                </div>
            </div>
        </div>
